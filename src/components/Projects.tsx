...
import { Plus, Search, Trash2, Edit2, Construction, Calendar, CheckCircle2, Clock, AlertCircle, FileSpreadsheet, Upload, X, Save } from 'lucide-react';
...
export default function Projects() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', description: '', status: 'active', startDate: new Date().toISOString().split('T')[0], endDate: '' });
    } catch (error) {
  ...
  return (
    <div className="space-y-10 pb-20" dir="rtl">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black text-foreground tracking-tight">إدارة المشاريع الإنشائية</h2>
          <p className="text-secondary mt-2 font-medium">متابعة دقيقة للمشاريع، المخططات، والتدفقات الزمنية</p>
        </div>
        <button 
          onClick={() => { setEditingProject(null); setFormData({ name: '', description: '', status: 'active', startDate: new Date().toISOString().split('T')[0], endDate: '' }); setIsModalOpen(true); }}
      ...
      <div className="card-lux p-6">
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder="بحث عن مشروع بالاسم أو الوصف..." 
            value={searchTerm} 
          ...
          >
            <div className="p-8 space-y-6 flex-1 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="bg-highlight/10 p-4 rounded-2xl border border-highlight/20 shadow-xl group-hover:scale-110 transition-transform">
                  <Construction className="text-highlight" size={28} />
                </div>
              ...
              <div className="relative z-10">
                ...
                <p className="text-secondary text-sm font-medium leading-relaxed mt-2 line-clamp-2">{project.description}</p>
              </div>

              <div className="flex items-center gap-6 text-xs font-black text-secondary relative z-10 uppercase tracking-widest">
                <div className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-lg border border-border"><Calendar size={14} className="text-highlight" /> {project.startDate}</div>
                {project.endDate && <div className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-lg border border-border"><CheckCircle2 size={14} className="text-primary" /> {project.endDate}</div>}
              </div>

              {/* Files Section */}
              <div className="pt-6 border-t border-border relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-[10px] font-black text-secondary uppercase tracking-[0.2em]">المستندات والمخططات</h4>
                  <label className="cursor-pointer bg-highlight/10 p-2 rounded-xl text-highlight hover:bg-highlight/20 transition-all border border-highlight/20 shadow-lg">
                    <Upload size={16} />
                    <input type="file" className="hidden" onChange={(e) => handleFileUpload(e, project.id!)} />
                  </label>
                </div>
                <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                  {files.filter(f => f.linkedEntityId === project.id).map(file => (
                  ...
                  {files.filter(f => f.linkedEntityId === project.id).length === 0 && (
                    <p className="text-[10px] text-secondary text-center italic py-2">لا توجد ملفات مرفوعة حالياً</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="p-6 bg-background/50 border-t border-border flex justify-between gap-4">
              <button onClick={() => { setEditingProject(project); setFormData(project); setIsModalOpen(true); }} className="flex-1 flex items-center justify-center gap-2 py-3 bg-highlight/10 text-highlight hover:bg-highlight/20 rounded-2xl transition-all font-black text-xs border border-highlight/20"><Edit2 size={16} /> تعديل المشروع</button>
              <button onClick={() => deleteDoc(doc(db, 'projects', project.id!))} className="flex-1 flex items-center justify-center gap-2 py-3 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-2xl transition-all font-black text-xs border border-red-500/20"><Trash2 size={16} /> حذف</button>
            </div>
          </motion.div>
        ))}
      </div>
              ...
              <div className="p-8 border-b border-border flex justify-between items-center bg-background/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-secondary font-medium uppercase tracking-widest mt-1">محرر البيانات الذكي</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-auto p-10 custom-scrollbar">
                {excelData.length > 0 ? (
                  ...
                  <div className="text-center py-32">
                    ...
                    <p className="text-secondary font-black text-xl">لا توجد بيانات متاحة للعرض</p>
                  </div>
                )}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-card rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] w-full max-w-2xl overflow-hidden border border-border">
              <div className="p-8 border-b border-border flex justify-between items-center bg-background/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-foreground">{editingProject ? 'تعديل بيانات المشروع' : 'إطلاق مشروع جديد'}</h3>
                    <p className="text-xs text-secondary font-medium uppercase tracking-widest mt-1">تخطيط المشاريع الإنشائية</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-8">
                <div className="space-y-3">
                  <label className="text-xs font-black text-secondary uppercase tracking-widest mr-1">اسم المشروع</label>
                  <input required type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="form-input-lux" placeholder="أدخل اسم المشروع..." />
                </div>
                <div className="space-y-3">
                  <label className="text-xs font-black text-secondary uppercase tracking-widest mr-1">وصف وتفاصيل المشروع</label>
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="form-input-lux h-32 py-4" placeholder="اكتب تفاصيل المشروع هنا..." />
                </div>
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-xs font-black text-secondary uppercase tracking-widest mr-1">تاريخ انطلاق المشروع</label>
                    <input required type="date" value={formData.startDate} onChange={(e) => setFormData({ ...formData, startDate: e.target.value })} className="form-input-lux" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-black text-secondary uppercase tracking-widest mr-1">حالة المشروع الحالية</label>
                ...
                <div className="flex justify-end gap-6 pt-8 border-t border-border">
                  ...
                  <button type="submit" disabled={loading} className="btn-primary px-16">
                    {loading ? 'جاري الحفظ...' : 'حفظ بيانات المشروع'}
                  </button>
                </div>
              </form>
            </motion.div>
...
function StatusBadge({ status }: { status: string | undefined }) {
  const styles = {
    ...
    completed: 'bg-highlight/10 text-highlight border-highlight/20',
    on_hold: 'bg-orange-500/10 text-orange-400 border-orange-500/20'
  };
  const labels = { active: 'نشط', completed: 'مكتمل', on_hold: 'متوقف' };
  return (
    <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border ${styles[status as keyof typeof styles]}`}>
